...
class Connection:
    ...
    :param load_balance_mode: load balance mode.
    :param ip_port_lists: host lists.
    ...
    def __init__(
        ...
        *,
        ...
        self.ip_port_lists=ip_port_lists
        ...
        if defer_connect:
            ...
            if load_balance_mode:
                self.call_load_balance()
            else:
                self.connect()

    def call_load_balance(self):
        method = getattr(self, f'_load_balance_{self.load_balance_mode.lower()}', None)
        if method:
            method()
        else:
            raise err.OperationalError(f"Load balance mode: {self.load_balance_mode} is not support.")

    def _load_balance_random(self):
        host = self.host
        port = self.port
        import random
        hosts = self.ip_port_lists.split(',')
        while(hosts):
            random.shuffle(hosts)
            random_element = hosts.pop()
            self.host, random_port = random_element.rsplit(':',1)
            self.port = int(random_port)
            try:
                self.connect()
                return
            except Exception as e:
                print(e)
                print(f"Failed to establish connections to host: {random_element}.")
        if host and port:
            try:
                self.host = host
                self.port = port
                ...
                print(e)
                print(f"Failed to establish connections to host: {host}:{port}.")
        if self.ip_port_lists:
            raise err.OperationalError(f"Failed to establish connections to all IP:port pairs in ip_port_lists: {self.ip_port_lists}.")
        raise err.OperationalError(f"Failed to establish connections to all host, please check param load_balance_mode,ip_port_lists, host,port.")
